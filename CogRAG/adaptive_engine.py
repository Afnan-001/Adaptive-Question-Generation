...
from CogRAG.question_generator import generate_question

# ----------------------------
# ✅ Files
# ----------------------------
MASTERY_FILE = Path("data/mastery_scores.json")
GRAPH_FILE = Path("graph_model/knowledge_graph.gml")
CURRENT_CONCEPT_FILE = Path("data/current_concept.txt")
RECENT_HISTORY_FILE = Path("data/recent_concepts.json")
...
def process_answer(concept, question_data, user_answer):
    ...
    recommendation = {
        "recommendation": f"Continue practicing {concept}",
        "reason": "This fallback guidance is used because no separate recommendation engine is implemented in this module.",
        "p_know": updated["p_know"]
    }
